...
contract('Lottery', function ([deployer, user1, user2]){
    ...
    describe('Distribute', function (){
        describe('When the answer is checkable', function (){
            ...
            it('should give the user the amount he or she bet when a single character matches', async () => {
                ...
                let user1BalanceAfter  = await web3.eth.getBalance(user1); // before + 0.005
    ...
    describe('isMatch', function (){
        let blockHash = '0xab0c2c0d480f0b34c91e9b07cb172aef9407a5e038baa6da6bd613acbcac7881';
        it('두개의 문자가 같았을 때 BettingResult.Win을 리턴', async () => {
            let matchingResult = await lottery.isMatch('0xab', blockHash);
            assert.equal(matchingResult, 1);
        });
        it('두개의 문자가 틀렸을 때 BettingResult.Fail을 리턴', async () => {
            let matchingResult = await lottery.isMatch('0xcd', blockHash);
            assert.equal(matchingResult, 0);
        });
        it('한개의 문자가 같았을 때 BettingResult.Draw을 리턴', async () => {
